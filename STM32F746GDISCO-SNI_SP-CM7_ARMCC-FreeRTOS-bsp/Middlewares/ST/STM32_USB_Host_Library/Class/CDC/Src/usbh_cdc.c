...
static USBH_StatusTypeDef CDC_SetControlLineState(USBH_HandleTypeDef *phost, 
                                                 CDC_LineStateTypeDef *linestate);
...
static USBH_StatusTypeDef USBH_CDC_Process (USBH_HandleTypeDef *phost)
{
  ...
  case CDC_SET_CONTROL_LINE_STATE:
    ...
    req_status = CDC_SetControlLineState(phost, CDC_Handle->pUserLineState);
...
/**
  ...
  * @retval USBH_StatusTypeDef : USB ctl xfer status
  */
static USBH_StatusTypeDef CDC_SetControlLineState(USBH_HandleTypeDef *phost, CDC_LineStateTypeDef *linestate)
{
  phost->Control.setup.b.bmRequestType = USB_H2D | USB_REQ_TYPE_CLASS | \
                              USB_REQ_RECIPIENT_INTERFACE;
  
  phost->Control.setup.b.bRequest = CDC_SET_CONTROL_LINE;
  phost->Control.setup.b.wValue.w = 0;
	
	if (linestate->b.activate_carrier) {
		phost->Control.setup.b.wValue.w |= CDC_ACTIVATE_CARRIER_SIGNAL_RTS;
	}
	if (linestate->b.dtePresent) {
...
USBH_StatusTypeDef USBH_CDC_SetLineCoding(USBH_HandleTypeDef *phost, CDC_LineCodingTypeDef *linecodin)
{
  ...
  if(phost->gState == HOST_CLASS && ((CDC_Handle->state == CDC_IDLE_STATE) || (CDC_Handle->state == CDC_TRANSFER_DATA)))
  {
...
* @retval None
*/
USBH_StatusTypeDef USBH_CDC_SetLineState(USBH_HandleTypeDef *phost, CDC_LineStateTypeDef *linestate)
{
  USBH_StatusTypeDef Status = USBH_BUSY;
  CDC_HandleTypeDef *CDC_Handle =  (CDC_HandleTypeDef*) phost->pActiveClass->pData;
  ...
  if(phost->gState == HOST_CLASS && ((CDC_Handle->state == CDC_IDLE_STATE) || (CDC_Handle->state == CDC_TRANSFER_DATA)))
  {
    CDC_Handle->state = CDC_SET_CONTROL_LINE_STATE;
    CDC_Handle->pUserLineState = linestate;
    Status = USBH_OK;
