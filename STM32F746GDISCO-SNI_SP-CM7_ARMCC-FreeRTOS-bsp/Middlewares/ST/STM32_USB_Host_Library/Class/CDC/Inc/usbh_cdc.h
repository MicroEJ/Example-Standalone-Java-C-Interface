...
#define CONTROL_LINESTATE_STRUCTURE_SIZE                        0x10
/**
  * @}
  */ 
...
{
  uint8_t Array[CONTROL_LINESTATE_STRUCTURE_SIZE];
  
  struct
  {
    uint8_t    dtePresent;                    /* Indicates to DCE if DTE is
                                                  present or not.
    ...
    uint8_t     activate_carrier;              /* Carrier control for half
                                               duplex modems.
                                               This signal corresponds to
...
typedef struct _CDC_Process
{
  ...
  CDC_LineStateTypeDef              *pUserLineState;
...
USBH_StatusTypeDef USBH_CDC_SetLineState(USBH_HandleTypeDef *phost, 
                                         CDC_LineStateTypeDef *linestate);
