...
static uint8_t get_format(uint32_t gc)
{
	// get image format, TODO work item 14458
	uint8_t format = LLDISPLAY_UTILS_getFormat(gc);
...
#if LLDISPLAY_BPP == 24
		format = RGB888_ID;
#elif LLDISPLAY_BPP == 16
		format = RGB565_ID;
#else
		#error "Invalid LCD pixel format"
#endif		
	}
...
void Java_com_is2t_microui_util_Grayscale_convertToGrayScale (uint32_t src, uint32_t dest, uint32_t w, uint32_t h)
{
	uint32_t src_addr = LLDISPLAY_UTILS_getBufferAddress(src);
	uint32_t dest_addr = LLDISPLAY_UTILS_getBufferAddress(dest);
