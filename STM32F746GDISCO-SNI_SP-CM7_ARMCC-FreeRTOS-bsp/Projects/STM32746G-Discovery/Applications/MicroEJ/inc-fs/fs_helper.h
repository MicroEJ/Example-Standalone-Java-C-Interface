 ...
 *
 * This module is used to delegate Fs processing to
 * an other native task (aim non block Java thread).
 * It uses a standard mailbox method, with event to specified
 * what the caller wants to process in Fs API.
 *
 * To have a centralized memory managment this module provide
 * a pool event which can be access by :
 * @li FS_get_event
 * @li FS_post_event
 * @li FS_release_event
 * @li FS_retrieve_event
 *
 * Typical sequence :
 *  @li task is created.
 *  @li task wait a entry event. (task is idle)
 *  @li jvm ask an free event 
 *  @li jvm post an event filled (at this point the module will suspend java thread) 
 *  @li an event is received (task process the event)
 *  @li at the end of processing the task resume the original Java thread caller.
 *  @li jvm will ask retrieve event (according to the thread id)
 *  @li jvm must release the event after use the output context
 *
 * This module uses an adaptation layer for implementation specific functions (os dependant).
 *
 */
