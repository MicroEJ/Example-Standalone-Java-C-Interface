...
/* Private functions ---------------------------------------------------------*/
static uint8_t* get_ethernet_mac_addr()
{
	// set MAC address to the default value
	uint8_t default_macaddress[6]= { MAC_ADDR0, MAC_ADDR1, MAC_ADDR2, MAC_ADDR3, MAC_ADDR4, MAC_ADDR5 };

    // check MAC address configuration requested by MicroEJ workbench extension
	int32_t mac_conf_enabled = ((int32_t)&LLNET_MAC_CONF == LLNET_TRUE);
	if(mac_conf_enabled){
		// MAC address configuration overriden by workbench extension
		parseMacAddr(LLNET_getMACAddr(), &macaddress[0], 0, 6);

		if(macaddress[3] == 0 && macaddress[4] == 0 && macaddress[5] == 0){
			// Consider only the prefix and generate an ID based on the mcu unique identifier
			uint32_t crc24 = computeCRC24(UNIQUE_DEVICE_ID_ADDRESS, UNIQUE_DEVICE_ID_SIZE);
			macaddress[3] = crc24 & 0xFF;
			macaddress[4] = (crc24>>8) & 0xFF;
			macaddress[5] = (crc24>>16) & 0xFF;
		}
	}else{
		// set default MAC address
		memcpy(macaddress, default_macaddress, 6);
	}
...
{
  uint8_t* macaddress = get_ethernet_mac_addr();
  ...
  xTaskCreate((TaskFunction_t) ethernetif_input, (const char*) "EthIf", INTERFACE_THREAD_STACK_SIZE, netif,
              INTERFACE_THREAD_PRIORITY,NULL);
